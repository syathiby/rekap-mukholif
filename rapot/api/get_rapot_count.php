...
while ($row = $result->fetch_assoc()) {
    ...
    if ($b_ta_start == $tahun_ajaran_start) {
        $count++;
        if ($edit_id > 0 && $row['id'] == $edit_id) {
            $is_editing_current_ta = true;
        }
    }
}
$stmt->close();
...
<div class="card bg-light border-0 shadow-sm mt-2">
    <div class="card-body py-2 px-3">
        <div class="d-flex justify-content-between align-items-center mb-1">
            <span class="small fw-bold text-secondary">Progress Rapot Tahun Ajaran <?php echo $tahun_ajaran_label; ?></span>
            <span class="small fw-bold <?php echo $count >= 12 ? 'text-danger' : 'text-primary'; ?>"><?php echo $count; ?> / 12</span>
        </div>
