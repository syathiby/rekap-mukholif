...
$filter_jp = $_GET['jenis_pelanggaran'] ?? 'semua'; // Filter baru
...
// Query baru untuk dropdown jenis pelanggaran
$jp_result = $conn->query("SELECT DISTINCT jenis_pelanggaran_id, jenis_pelanggaran_nama FROM arsip_data_pelanggaran WHERE arsip_id = $arsip_id AND tipe = 'Umum' AND jenis_pelanggaran_nama IS NOT NULL ORDER BY jenis_pelanggaran_nama ASC");


// --- QUERY UTAMA DENGAN FILTER BARU ---
$sql_data = "
    SELECT
        santri_id,
        santri_nama,
...
if ($filter_jp !== 'semua') {
    $sql_data .= " AND jenis_pelanggaran_id = ?";
    $params_data[] = $filter_jp;
    $types_data .= "i";
}
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            ...
            <form method="get" id="filterForm" class="row g-3">
                ...
                <div class="col-lg-3 col-md-6">
                    <label for="jenis_pelanggaran" class="form-label small">Jenis Pelanggaran</label>
                    <select name="jenis_pelanggaran" id="jenis_pelanggaran" class="form-select">
                        <option value="semua">Semua Jenis</option>
                        <?php mysqli_data_seek($jp_result, 0); while($jp = $jp_result->fetch_assoc()): ?>
                            <option value="<?= htmlspecialchars($jp['jenis_pelanggaran_id']) ?>" <?= ($filter_jp == $jp['jenis_pelanggaran_id']) ? 'selected' : '' ?>>
                                <?= htmlspecialchars($jp['jenis_pelanggaran_nama']) ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
                    ...
                    <tbody>
                        <?php if($data_rekap->num_rows > 0): ?>
                            ...
                            <tr class="rank-<?= $no ?>">
                                ...
                                <td class="text-center">
                                    <a href="detail_pelanggaran_arsip.php?arsip_id=<?= $arsip_id ?>&santri_id=<?= $row['santri_id'] ?>&<?= $filter_params ?>" class="btn btn-sm btn-detail rounded-pill px-3">
