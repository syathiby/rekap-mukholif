...
// Cek apakah parameter id dan kamar ada di URL
$arsip_id = (int)($_GET['id'] ?? 0);
if ($arsip_id <= 0) {
    die('<div class="content-wrapper"><div class="container-fluid"><div class="alert alert-danger">Arsip tidak valid.</div></div></div>');
}

if (!isset($_GET['kamar']) || empty($_GET['kamar'])) {
    die('<div class="content-wrapper"><div class="container-fluid"><div class="alert alert-danger">Nama kamar tidak ditemukan.</div></div></div>');
}
...
if (!$arsip) {
    die('<div class="content-wrapper"><div class="container-fluid"><div class="alert alert-danger">Arsip tidak ditemukan.</div></div></div>');
}

// Ambil data pelanggaran kebersihan
$sql_pelanggaran = "SELECT id, tanggal, catatan FROM arsip_data_pelanggaran_kebersihan WHERE arsip_id = ? AND kamar = ? ORDER BY tanggal DESC";
$stmt_pelanggaran = $conn->prepare($sql_pelanggaran);
$stmt_pelanggaran->bind_param("is", $arsip_id, $nama_kamar);
...
$stmt_trend->bind_param("is", $arsip_id, $nama_kamar);
$stmt_trend->execute();
...
<div class="container">
    <div class="header-card">
        <h2><i class="fas fa-door-open"></i> Detail Kamar <?= $nama_kamar ?> (Arsip)</h2>
    ...
    <div class="info-card filter-info">
        <i class="fas fa-archive" style="color: var(--secondary); font-size: 1.5rem;"></i>
        <div>
            <p style="font-size: 1rem; margin: 0; font-weight: 400;">
                Menampilkan riwayat pelanggaran untuk arsip: 
                <strong><?= htmlspecialchars($arsip['judul']) ?></strong> (<?= date('d M Y', strtotime($arsip['tanggal_mulai'])) ?> - <?= date('d M Y', strtotime($arsip['tanggal_selesai'])) ?>)
            </p>
        </div>
    </div>
