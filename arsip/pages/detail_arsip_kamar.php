...
if ($arsip_id <= 0 || empty($kamar)) {
    die("<div class='container mt-5'><div class='alert alert-danger'>Pilih kamar dan arsip terlebih dahulu.</div></div>");
}
...
$sql_summary = "
    SELECT 
        COUNT(s.santri_id) AS jumlah_santri,
        COALESCE(SUM(pel.total_pelanggaran), 0) / COUNT(s.santri_id) AS avg_pelanggaran,
        COALESCE(SUM(pel.kasus), 0) / COUNT(s.santri_id) AS avg_kasus,
        COALESCE(SUM(rwd.total_reward), 0) / COUNT(s.santri_id) AS avg_reward,
        COALESCE(SUM(rpt.avg_rapot), 0) / COUNT(s.santri_id) AS avg_rapot,
        (COALESCE(SUM(pel.total_pelanggaran), 0) - COALESCE(SUM(rwd.total_reward), 0)) / NULLIF(COUNT(s.santri_id), 0) AS avg_poin_bersih,
        COALESCE(kbs.total_kebersihan, 0) AS pelanggaran_kebersihan
    FROM arsip_data_santri s
    LEFT JOIN (
        SELECT santri_id, COUNT(id) AS kasus, SUM(poin) AS total_pelanggaran
        FROM arsip_data_pelanggaran 
        WHERE arsip_id = ? AND tipe = 'Umum' GROUP BY santri_id
    ) pel ON s.santri_id = pel.santri_id
    LEFT JOIN (
        SELECT santri_id, SUM(poin_reward) AS total_reward
        FROM arsip_data_reward
        WHERE arsip_id = ? GROUP BY santri_id
    ) rwd ON s.santri_id = rwd.santri_id
    LEFT JOIN (
        SELECT santri_id, 
        ...
        WHERE arsip_id = ?
        GROUP BY santri_id
    ) rpt ON s.santri_id = rpt.santri_id
    LEFT JOIN (
        SELECT kamar, COUNT(*) AS total_kebersihan
        FROM arsip_data_pelanggaran_kebersihan
        WHERE arsip_id = ? AND kamar = ?
        GROUP BY kamar
    ) kbs ON s.santri_kamar = kbs.kamar
    WHERE s.arsip_id = ? AND s.santri_kamar = ?
    GROUP BY s.santri_kamar
";
$stmt_sum = mysqli_prepare($conn, $sql_summary);
mysqli_stmt_bind_param($stmt_sum, "iiiisis", $arsip_id, $arsip_id, $arsip_id, $arsip_id, $kamar, $arsip_id, $kamar);
