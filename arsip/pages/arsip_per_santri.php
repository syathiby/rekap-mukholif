...
if ($arsip_id <= 0) die("ID Arsip tidak valid.");
...
$tipe = in_array($tipe_raw, ['semua', 'daftar_hitam', 'peringkat']) ? $tipe_raw : 'semua';
...
if ($tipe === 'semua') {
    ...
    if ($filter_kamar) { $sql .= " AND s.santri_kamar = ?"; $params[] = $filter_kamar; $types .= "s"; }
    if ($filter_kelas)  { $sql .= " AND s.santri_kelas = ?"; $params[] = $filter_kelas;  $types .= "s"; }
    
    // Sort kamar (numeric then string), kelas (numeric then string), nama
    $sql .= " ORDER BY CAST(REGEXP_REPLACE(s.santri_kamar, '[^0-9]', '') AS UNSIGNED) ASC, REGEXP_REPLACE(s.santri_kamar, '[0-9]', '') ASC, CAST(REGEXP_REPLACE(s.santri_kelas, '[^0-9]', '') AS UNSIGNED) ASC, REGEXP_REPLACE(s.santri_kelas, '[0-9]', '') ASC, s.santri_nama ASC";
    ...
    $sql = "SELECT s.santri_id AS id, s.santri_nama AS nama, s.santri_kelas AS kelas, s.santri_kamar AS kamar, s.total_poin_saat_arsip AS poin_aktif,
            ...
            JOIN (
                  SELECT p.santri_id, 
                         ...
                         GROUP_CONCAT(CONCAT(p.jenis_pelanggaran_nama, ':', p.cnt) SEPARATOR '|') AS detail_kasus
                  FROM (
                      SELECT santri_id, jenis_pelanggaran_id, jenis_pelanggaran_nama, COUNT(id) as cnt, SUM(poin) as tot_poin
                      FROM arsip_data_pelanggaran 
                      WHERE arsip_id = ? AND tipe = 'Umum' ";
    
    $params = [$arsip_id]; $types = "i";
    if ($filter_jp)       { $sql .= " AND jenis_pelanggaran_id = ?"; $params[] = $filter_jp;       $types .= "i"; }
    if ($filter_bagian)   { $sql .= " AND bagian = ?";              $params[] = $filter_bagian;   $types .= "s"; }
    
    $sql .= "         GROUP BY santri_id, jenis_pelanggaran_id, jenis_pelanggaran_nama
                  ) p
                  GROUP BY p.santri_id
            ) AS sub ON s.santri_id = sub.santri_id WHERE s.arsip_id = ?";
            
    $params[] = $arsip_id; $types .= "i";
    
    if ($filter_kamar) { $sql .= " AND s.santri_kamar = ?"; $params[] = $filter_kamar; $types .= "s"; }
    if ($filter_kelas)  { $sql .= " AND s.santri_kelas = ?"; $params[] = $filter_kelas;  $types .= "s"; }
    $sql .= " AND sub.total_pelanggaran_periode > 0 ORDER BY total_poin_periode DESC, total_pelanggaran_periode DESC, s.santri_nama ASC";
...
if ($tipe === 'peringkat') {
    ...
    if ($formula === 'neraca') {
        ...
        $sql = "SELECT s.santri_id AS id, s.santri_nama AS nama, s.santri_kelas AS kelas, s.santri_kamar AS kamar,
                       ...
                       0 AS avg_rapot
                FROM arsip_data_santri s
                LEFT JOIN (SELECT santri_id, COUNT(id) AS total_pelanggaran_periode, SUM(poin) AS total_poin_pelanggaran
                           FROM arsip_data_pelanggaran
                           WHERE arsip_id = ? AND tipe = 'Umum' GROUP BY santri_id) AS sub_p ON s.santri_id = sub_p.santri_id
                LEFT JOIN (SELECT santri_id, COUNT(id) AS total_reward_periode, SUM(poin_reward) AS total_poin_reward
                           FROM arsip_data_reward
                           WHERE arsip_id = ? GROUP BY santri_id) AS sub_r ON s.santri_id = sub_r.santri_id
                WHERE s.arsip_id = ?";

        $params = [$arsip_id, $arsip_id, $arsip_id]; $types = "iii";
        if ($filter_kamar) { $sql .= " AND s.santri_kamar = ?"; $params[] = $filter_kamar; $types .= "s"; }
        if ($filter_kelas)  { $sql .= " AND s.santri_kelas = ?"; $params[] = $filter_kelas;  $types .= "s"; }
        $sql .= " AND (sub_p.total_poin_pelanggaran > 0 OR sub_r.total_poin_reward > 0)";
        $sql .= ($sort_order === 'terburuk')
            ? " ORDER BY skor DESC, total_pelanggaran DESC, s.santri_nama ASC LIMIT 200"
            : " ORDER BY skor ASC, total_pelanggaran ASC, s.santri_nama ASC LIMIT 200";
        ...
        if (!$hide_violators) {
            $sql .= "LEFT JOIN (SELECT santri_id, COUNT(id) AS jml_pelanggaran, SUM(poin) AS total_pelanggaran
                                FROM arsip_data_pelanggaran
                                WHERE arsip_id = ? AND tipe = 'Umum' GROUP BY santri_id) pel ON s.santri_id = pel.santri_id ";
        }
        $sql .= "LEFT JOIN (SELECT santri_id, SUM(poin_reward) AS total_reward
                             FROM arsip_data_reward
                             WHERE arsip_id = ? GROUP BY santri_id) rwd ON s.santri_id = rwd.santri_id
                 LEFT JOIN (SELECT santri_id,
                             ...
                             FROM arsip_data_rapot WHERE arsip_id = ? GROUP BY santri_id) rpt ON s.santri_id = rpt.santri_id
                 WHERE s.arsip_id = ? ";

        $params = []; $types = "";
        if (!$hide_violators) { $params[] = $arsip_id; $types .= "i"; }
        $params[] = $arsip_id; $params[] = $arsip_id; $params[] = $arsip_id; $types .= "iii";
        
        if (!empty($filter_kamar)) { $sql .= " AND s.santri_kamar = ?"; $params[] = $filter_kamar; $types .= "s"; }
        if (!empty($filter_kelas))  { $sql .= " AND s.santri_kelas = ?"; $params[] = $filter_kelas;  $types .= "s"; }
        if ($hide_violators) {
            $sql .= " AND s.santri_id NOT IN (SELECT santri_id FROM arsip_data_pelanggaran WHERE arsip_id = ? AND tipe = 'Umum')";
            $params[] = $arsip_id; $types .= "i";
        }
...
<div class="container py-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 gap-3">
        <div>
            ...
            <p class="text-muted mb-0" style="font-size:.9rem;">Arsip: <strong><?= htmlspecialchars($arsip['judul']) ?></strong></p>
        </div>
        <div>
                ...
                <div class="col-lg-2 col-md-4 col-6">
                    <div class="filter-group">
                        ...
                        <select name="tipe" id="tipeSelect" class="form-select fw-semibold text-primary">
                            <option value="semua"        <?= ($tipe==='semua')       ?'selected':'' ?>>📋 Data Semua Santri</option>
...
<?php
$filter_qs = "";
if (!empty($tipe)) $filter_qs .= "&tipe=" . urlencode($tipe);
...
<div class="mb-3 text-muted fw-semibold">
    Total Santri Terarsip: <strong class="text-primary"><?= mysqli_num_rows($query) ?></strong> Santri
</div>
<div class="card border-0 shadow-sm">
    <div class="card-body table-responsive p-0">
        <table class="table table-hover mb-0" id="tabelSemua">
            <thead>
                <tr>
            ...
            <?php if (mysqli_num_rows($query) === 0): ?>
                <tr><td colspan="4" class="text-center py-5 text-muted">
                    ...
                    <p class="mb-0">Tidak ada data santri yang ditemukan.</p>
                </td></tr>
            <?php else: $no = 1; while ($row = mysqli_fetch_assoc($query)): ?>
                <tr class="searchable-row" data-nama="<?= strtolower(htmlspecialchars($row['nama'])) ?>" style="cursor: pointer;" onclick="window.location.href='detail_arsip_santri.php?arsip_id=<?= $arsip_id ?>&id=<?= $row['id'] ?><?= $filter_qs ?? '' ?>'">
                    <td class="text-center text-muted"><?= $no++ ?></td>
                    <td class="ps-4 fw-bold text-dark"><?= htmlspecialchars($row['nama']) ?></td>
                    <td class="text-center"><?= htmlspecialchars($row['kelas'] ?? '-') ?></td>
                    <td class="text-center"><?= htmlspecialchars($row['kamar'] ?? '-') ?></td>
                </tr>
            <?php endwhile; endif; ?>
            </tbody>
        </table>
    ...
    <div class="card-body table-responsive p-0">
        <table class="table table-hover mb-0" id="tabelHitam">
            ...
            <?php else: ?>
                ...
                <tr class="rank-<?= $no ?> hover-row" data-nama="<?= strtolower(htmlspecialchars($row['nama'])) ?>" onclick="window.location.href='detail_arsip_santri.php?arsip_id=<?= $arsip_id ?>&id=<?= $row['id'] ?><?= $filter_qs ?>'" style="cursor: pointer;">
...
if (empty($santri_data)) {
    ...
    foreach ($limit as $row):
        ...
        $onclick = "window.location.href='detail_arsip_santri.php?arsip_id=" . $arsip_id . "&id=" . $row['id'] . $filter_qs . "'";
?>
