...
if ($arsip_id <= 0) {
    die("<div class='container' style='padding-top:20px;'><div class='alert alert-danger'>⚠ Arsip tidak valid.</div></div>");
}
...
if (!$arsip) {
    die("<div class='container' style='padding-top:20px;'><div class='alert alert-danger'>⚠ Arsip tidak ditemukan.</div></div>");
}

// 🔹 Kueri 1: Master Kamar (O(1))
$q_master = mysqli_query($conn, "SELECT santri_kamar AS kamar, COUNT(santri_id) AS jumlah_santri FROM arsip_data_santri WHERE arsip_id = $arsip_id AND santri_kamar IS NOT NULL AND santri_kamar != '' GROUP BY santri_kamar");
$raw_data = [];
...
$stmt_pel = mysqli_prepare($conn, "SELECT s.santri_kamar AS kamar, COUNT(p.id) AS total_kasus, SUM(p.poin) AS total_poin FROM arsip_data_pelanggaran p JOIN arsip_data_santri s ON p.santri_id = s.santri_id AND p.arsip_id = s.arsip_id WHERE p.arsip_id = ? AND p.tipe = 'Umum' GROUP BY s.santri_kamar");
mysqli_stmt_bind_param($stmt_pel, "i", $arsip_id);
...
$stmt_rwd = mysqli_prepare($conn, "SELECT s.santri_kamar AS kamar, SUM(dr.poin_reward) AS total FROM arsip_data_reward dr JOIN arsip_data_santri s ON dr.santri_id = s.santri_id AND dr.arsip_id = s.arsip_id WHERE dr.arsip_id = ? GROUP BY s.santri_kamar");
mysqli_stmt_bind_param($stmt_rwd, "i", $arsip_id);
...
$stmt_rpt = mysqli_prepare($conn, "SELECT s.santri_kamar AS kamar, SUM(((puasa_sunnah + sholat_duha + sholat_malam + sedekah + sunnah_tidur + ibadah_lainnya + lisan + sikap + kesopanan + muamalah + tidur + keterlambatan + seragam + makan + arahan + bahasa_arab + mandi + penampilan + piket + kerapihan_barang) / 20)) AS total FROM arsip_data_rapot r JOIN arsip_data_santri s ON r.santri_id = s.santri_id AND r.arsip_id = s.arsip_id WHERE r.arsip_id = ? GROUP BY s.santri_kamar");
mysqli_stmt_bind_param($stmt_rpt, "i", $arsip_id);
...
$stmt_kbs = mysqli_prepare($conn, "SELECT kamar, COUNT(*) AS total FROM arsip_data_pelanggaran_kebersihan WHERE arsip_id = ? GROUP BY kamar");
mysqli_stmt_bind_param($stmt_kbs, "i", $arsip_id);
        ...
        ?>
                <div class="kamar-card" style="animation-delay: <?= $animation_delay ?>s;">
                    ...
                    <div class="card-content">
                        <h3>
                            <a href="detail_arsip_kamar.php?id=<?= $arsip_id ?>&kamar=<?= urlencode($row['kamar']) ?>" class="stretched-link" style="color: inherit; text-decoration: none;">
